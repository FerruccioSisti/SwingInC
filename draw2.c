...
static void draw_brush (GtkWidget *widget, gdouble x, gdouble y)
{
  ...
  if (shape == 1)
  {
      // cairo_rectangle (cr, x - 3, y - 3, 12, 12);
      cairo_arc(cr, x - 3, y - 3, 12, 80, 80);
  }
  else
  {
      cairo_arc(cr, x - 3, y - 3, 12, 80, 80);
  ...
  gtk_widget_queue_draw_area (widget, x - 3, y - 3, 12, 12);
}
...
button_press_event_cb (GtkWidget *widget, GdkEventButton *event, gpointer data)
{
  ...
  if (event->button == GDK_BUTTON_PRIMARY)
    ...
    //if right click clear the screen
...
int main (int argc, char **argv)
{
  ...
  {
      shape = 1;
      app2 = gtk_application_new ("org.gtk.example", G_APPLICATION_FLAGS_NONE);
